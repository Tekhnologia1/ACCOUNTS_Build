...
const createPayment = async (from_gl_id, to_gl_id, from_firm_id, to_firm_id, user_id, amount) => {
  ...
  try {
    ...
    const insertTransactionQuery = 'INSERT INTO tbl_transactions (from_gl_id, to_gl_id, amount, from_firm_id, to_firm_id, user_id) VALUES (?, ?, ?, ?, ?, ?)';
    const result = await query(insertTransactionQuery, [from_gl_id, to_gl_id, amount, from_firm_id, to_firm_id, user_id], connection);
...
const getAllTransactionsByFirmId = async (from_firm_id, startDate, endDate) => {
  let sql = `
    SELECT 
      t.transaction_id, t.amount, t.transaction_date, t.from_gl_id, t.to_gl_id,
