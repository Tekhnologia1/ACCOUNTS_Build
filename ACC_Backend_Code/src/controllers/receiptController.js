...
const createReceipt = async (req, res) => {
  try {
    const { to_firm_id, to_gl_id, from_firm_id, from_gl_id, amount } = req.body;
    ...
    const transactionId = await createReceiptTransaction(to_firm_id, to_gl_id, from_firm_id, from_gl_id, user_id, amount);
...
const getReceiptsByFirm = async (req, res) => {
  try {
    ...
    // if (!to_firm_id) {
    //   return res.status(400).send({ status: false, message: "to_firm_id is required" });
    // }
